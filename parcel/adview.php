		...
		<div class="row"  >
			...
			<div class="table-responsive">
				...
				<table class="table" >
					...
					<?php 
       			//=$_SESSION['adminname'];
							$query = "SELECT* from consigne where admin='$admin' and status=1 order by id desc LIMIT ".$page_first_result.','.$result_per_page;
								$result = mysqli_query($conn, $query);//runs the connection to dsatabase and then run the query
								$return = mysqli_num_rows($result);
						// 
				if($return ){
								...
								while ($row = mysqli_fetch_assoc($result)){ 
									...
									<tr id="show<?php echo $id ?>">
										...
										<td><a href="hview?trk=<?php echo $newt ?>" class="btn btn-small btn-info">VIEW HISTORY</a></td>		
										...
										<td>
								  	<input type="hidden" id="viewid<?php echo $id ?>" value="<?php echo $id ?>"  name="">
