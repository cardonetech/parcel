...
<section id="main-content">
 	...
 	$query = "SELECT* from consignment where adminname='$admin' and status=1";
	...
	<section class="wrapper">
		...
		<div class="row"  >
			...
			<h4 style="text-align: center;">ALL CONSIGNMENT CREATED BY <?php echo $admin ?></h4>		
			<div class="col-md-12 col-lg-12 col-md-push-0">
			<div class="table-responsive">
				...
				<table class="table" >
					...
					<?php 
       			//=$_SESSION['adminname'];
							$query = "SELECT* from consigne where admin='$admin' and status=0 order by id desc LIMIT ".$page_first_result.','.$result_per_page;
								$result = mysqli_query($conn, $query);//runs the connection to dsatabase and then run the query
								$return = mysqli_num_rows($result);
						// 
				if($return ){
								...
								while ($row = mysqli_fetch_assoc($result)){ 
									...
									<tr id="show<?php echo $id ?>">
										...
										<td><a href="packview?trk=<?php echo $newt ?>" class="btn btn-small btn-info">VIEW</a></td>			
													

										<td><a href="addhistory?cst=<?php echo $newt ?>" class="btn btn-small btn-success">ADD HISTORY</a></td>
										<td><a href="getreceipt?rcp=<?php echo $newt ?>" class="btn btn-small btn-info">RECEIPT</a></td>


										<td>
								  	<input type="hidden" id="retrieve<?php echo $id ?>" value="<?php echo $id ?>"  name="">
								  	
								  	<button id="delview<?php echo $id ?>">Retrieve</button></td>
				...
				<script type="text/javascript">
				 ...
				 $("#delview<?php echo $id ?>").click(function(){
				 	 ...
				 	 var retrieve = $("#retrieve<?php echo $id ?> " ).val();
				 	
				 		var retval =confirm("do u want to retrieve <?php echo $recname ?> package");
				 		...
				 		if(retval==true){
				              ...
				              $.ajax({
				                    ...
				                    data:{
				                      ...
				                      beforeSend: function(){
				                $('#delview<?php echo $id ?>').val("Retrieving....");
				              },
				...
				for ($page=1; $page <=$number_of_pages ; $page++) { 
						echo ' <a class="btn-sm btn-info btn" href="deleted?page='.$page.'">'.$page.'</a> ';
					} ?>
				</div>;
