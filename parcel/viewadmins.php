...
<section id="main-content">
	...
	<div class="row"  >
		...
		<br>
			<span id="rst"></span>
			<h4 style="text-align: center;">ALL ADMINS</h4>		
		<div class="col-md-10 col-lg-10 col-md-push-1">
			<div class="table-responsive">
				<table class="table" >
					...
					<?php 
       //=$_SESSION['adminname'];
							$query = "SELECT* from admin where status=1 LIMIT ".$page_first_result.','.$result_per_page."";
								$result = mysqli_query($conn, $query);//runs the connection to dsatabase and then run the query
								$return = mysqli_num_rows($result);
						// 
							if($return ){
								...
								while ($row = mysqli_fetch_assoc($result)){ 
									...
									<tr id="show<?php echo $id ?>">
										...
										<td>
											<input type="hidden" id="adminid<?php echo $id ?>" value="<?php  echo $id ?>"  name="">
											<button class="btn btn-sm btn-danger" id="delview<?php echo $id ?>">DELETE</button>
										</td>					
...
<script type="text/javascript">
 ...
 $("#delview<?php echo $id ?>").click(function(){
 	 ...
 	 var adminid = $("#adminid<?php echo $id ?> " ).val();

 	 //alert(adminid);
 	
 		var retval =confirm("do u want to remove <?php echo $adminname ?> as admin");
