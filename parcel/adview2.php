		...
		<div class="row"  >
			...
			<div class="table-responsive">
				...
				<table class="table" >
					...
					<?php 
       			//=$_SESSION['adminname'];
							$query = "SELECT* from consignment where adminname='$admin' and status=1 order by id desc LIMIT ".$page_first_result.','.$result_per_page;
								$result = mysqli_query($conn, $query);//runs the connection to dsatabase and then run the query
								$return = mysqli_num_rows($result);
						// 
				if($return ){
				...
				for ($page=1; $page <=$number_of_pages ; $page++) { 
						echo ' <a class="btn-sm btn-info btn" href="view.php?page='.$page.'">'.$page.'</a> ';
					} ?>
				</div>;
